...
{
  if ( auto variable = create_variable( node.source_location, node.scope, node.name, node.var_decl_location ) )
  {
    node.variable = std::move( variable );
    visit_children( node );
  }
    ...
    const SourceLocation& var_decl_location )
{
  ...
  {
    return local_scope->create( maybe_scoped_name, WarnOn::Never, source_location, var_decl_location );
  }
  else
  {
